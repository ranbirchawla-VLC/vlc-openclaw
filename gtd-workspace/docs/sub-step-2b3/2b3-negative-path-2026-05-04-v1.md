...
Exact reply prose per branch; prose lands at build with TDD against the test corpus. Threshold for branch attribution edge cases (e.g., "hi, what's on my plate?" — greeting plus query? Recommendation: classifier routes to `query_tasks` since the substantive intent dominates; greeting is incidental). Build refines branch attribution through corpus.
