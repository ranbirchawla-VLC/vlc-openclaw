...
2. **`cycle_shortlist.csv`**: the wide bucket-row CSV. Primary
   strategy input. Every reference the analyzer scored, one row per
   (reference x bucket). All market math is done; strategy reads
   breadth and applies judgment.

   **Bucket-row schema awareness**: a single reference (e.g. Tudor
   79830RB) may appear across multiple rows when the analyzer has
   scored different keying axes (dial_numerals, auction_type,
   dial_color) separately. Treat each row as a distinct market signal,
   not a duplicate. Fields split into two levels:
   - **Reference-level** (repeat identically across a reference's
     bucket rows): `brand`, `model`, `trend_signal`, `trend_median_*`,
     `momentum_*`, `confidence_*`.
   - **Bucket-level** (vary across rows for the same reference):
     `signal`, `median`, `max_buy_nr`, `max_buy_res`, `st_pct`,
     `volume`, `risk_nr`, `capital_required_*`, `expected_net_*`.

   When discussing a reference with the operator, aggregate across its
   bucket rows: the reference has Strong + Normal buckets, or Arabic +
   unknown-numerals rows. Do not collapse or average bucket-level
   signals; surface the split so the operator can apply judgment.
...
4. **`analysis_cache.json`**: detail-on-demand. Every reference the
   agent knows about, with full per-bucket scoring. Use for individual
   reference deep-dives when the shortlist row doesn't surface enough
   context. Not the primary input; use `cycle_shortlist.csv` for
   breadth scanning.

5. **`cycle_focus.json`**: what the agent currently focuses on. Your
   `cycle_focus` decision either confirms, evolves, or replaces this.
...
- `targets`: **3–6 references** the operator will lead with. Each
  ...
  rejection signal for the operator. Source targets from
  `cycle_shortlist.csv` Strong/Normal bucket rows; reference each
  pick to the bucket that drove the selection.
- `capital_target`: deployable dollars this cycle. Pulls from the
  ...
  `cycle_focus.json` as a default and call it out in notes.
