...
**Purpose:** WatchTrack inventory lookups and status updates.

> ⚠️ **In the automated Vardalux pipeline, Step 0 (WatchTrack lookup) is always pre-completed by OpenClaw using its browser tool before Claude Code is spawned. `watchtrack.json` will already exist in the working folder. Claude Code must NOT attempt to do WatchTrack lookups — skip Step 0 entirely and start at Step 1.**
...
```
STEP 0: WATCHTRACK LOOKUP  ← ⚠️ PRE-COMPLETED BY OPENCLAW. DO NOT REPEAT.
│  watchtrack.json is already written to working_folder by OpenClaw.
│  Read it. Do not navigate to WatchTrack. Do not use Chrome or Peekaboo.
...
### Skill locations (OpenClaw workspace)

```
~/.openclaw/workspace/skills/watchtrack/
├── SKILL.md
└── references/
    └── watchtrack-config.json

~/.openclaw/workspace/skills/watch-listing-content/
├── SKILL.md
└── references/
    ├── platform-templates.md        ← copy-paste template structures, all 9 platforms
    ├── character-substitutions.md   ← Facebook algorithm avoidance table
    └── posting-checklist.md         ← universal + price-based + brand-specific routing
```

### How the orchestrator accesses reference files

When generating the full listing document (Step 4), read these files from
the `watch-listing-content` skill folder:
...
These files are part of the content skill, not the orchestrator. If the
content standards change (new platform added, substitution table updated,
pricing group changed), only the content skill files update. The orchestrator
never needs to change.

### WatchTrack config

The WatchTrack base URL and valid substatus options are stored in:

```
~/.openclaw/workspace/skills/watchtrack/references/watchtrack-config.json
```

If WatchTrack changes its URL or adds new substatus options, update this
file. The skill reads it before every execution.
