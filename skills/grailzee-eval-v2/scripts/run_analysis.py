...
def run_analysis(
    ...
    source_report = os.path.basename(current_csv)
    ...
    try:
        roll_cycle.run(
            ...
            output_path=os.path.join(
                os.path.dirname(cache_path), "cycle_outcome.json"
            ) if cache_path else None,
        )
    except Exception:
        pass  # cycle rollup failure is non-fatal
    ...
    # Step 15: Cache write
    market_window = {
        "pricing_reports": [os.path.basename(p) for p in csv_paths[:2]],
        "trend_reports": [os.path.basename(p) for p in csv_paths],
    }
    write_cache.run(
        all_results, trends, changes, breakouts,
...
def main() -> int:
    ...
    with tracer.start_as_current_span("run_analysis") as span:
        ...
        try:
            ...
            span.set_attribute("references_scored",
                               len(result.get("references", {})) if isinstance(result, dict) else 0)
